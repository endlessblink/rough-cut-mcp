...
function getClaudeConfigPath() {
  // Windows-only configuration path
  if (process.platform !== 'win32') {
    throw new Error('This installer only supports Windows. Claude Desktop is a Windows application.');
  }
  ...
  try {
    ...
    try {
      ...
      if (error.code === 'EACCES') {
        log('⚠️  Cannot create config directory', colors.yellow);
        log('Please ensure Claude Desktop is installed and you have permissions', colors.yellow);
    ...
    try {
      ...
      // If we can't write, show manual instructions
      if (writeError.code === 'EACCES') {
        log('\n⚠️  Cannot write to configuration file', colors.yellow);
    ...
    // Generate proper Windows paths for manual installation
    let serverPath = path.join(__dirname, 'build', 'index.js');
    let assetsPath = path.join(__dirname, 'assets');
