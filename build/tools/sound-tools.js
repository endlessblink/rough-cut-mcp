...
export function createSoundTools(config) {
    return [
        {
...
export function createSoundHandlers(config) {
    ...
    return {
        'search-sound-effects': async (args) => {
            if (!freesoundService) {
            ...
            try {
                ...
                const searchResponse = await freesoundService.searchSounds(validatedArgs.query, {
                    duration: validatedArgs.duration,
                    pageSize: validatedArgs.maxResults,
                });
                let downloadedSounds = [];
        ...
        'download-sound-effects': async (args) => {
            if (!freesoundService) {
        ...
        'search-popular-sounds': async (args) => {
            if (!freesoundService) {
            ...
            try {
                const sounds = await freesoundService.getPopularSounds(args.category, args.maxResults || 10);
        ...
        'advanced-sound-search': async (args) => {
            if (!freesoundService) {
...
//# sourceMappingURL=sound-tools.js.map