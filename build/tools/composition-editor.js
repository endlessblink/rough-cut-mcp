...
exports.CompositionEditor = void 0;
exports.registerCompositionTools = registerCompositionTools;
...
class CompositionEditor {
    ...
    constructor(server) {
        this.server = server;
        this.logger = server.baseLogger.service('composition-editor');
    }
    ...
    async addElement(projectPath, element) {
        const compositionFile = path.join(projectPath, 'src', 'VideoComposition.tsx');
        if (!await fs_extra_1.default.pathExists(compositionFile)) {
            throw new Error(`VideoComposition.tsx not found in project`);
        }
        let content = await fs_extra_1.default.readFile(compositionFile, 'utf8');
        ...
        content = (0, interpolation_validator_js_1.processVideoCode)(content);
        // Write back to file
        await fs_extra_1.default.writeFile(compositionFile, content);
    ...
    async editElement(projectPath, element) {
        const compositionFile = path.join(projectPath, 'src', 'VideoComposition.tsx');
        let content = await fs_extra_1.default.readFile(compositionFile, 'utf8');
    ...
    async removeElement(projectPath, elementId) {
        const compositionFile = path.join(projectPath, 'src', 'VideoComposition.tsx');
        let content = await fs_extra_1.default.readFile(compositionFile, 'utf8');
    ...
    generateTextElement(element) {
        ...
        const { text = 'Sample Text', fontSize = 48, color = '#ffffff', x = '50%', y = '50%', opacity = 1, fontFamily = 'Arial, sans-serif', fontWeight = 'normal', textAlign = 'center', animation = 'none' } = props;
  ...
  const ${id}X = safeInterpolate(frame, [${timing?.from || 0}, ${(timing?.from || 0) + 30}], [-100, 0], { extrapolateRight: 'clamp' });`;
    ...
    generateImageElement(element) {
        const { id, props, timing } = element;
        const { src, width = 400, height = 300, x = '50%', y = '50%', opacity = 1, objectFit = 'contain', borderRadius = 0, animation = 'none' } = props;
        const sequenceProps = timing
            ? `from={${timing.from}} durationInFrames={${timing.duration}}`
            : 'from={0} durationInFrames={90}';
        // Add animation logic
  ...
  const ${id}Scale = safeInterpolate(frame, [${timing?.from || 0}, ${(timing?.from || 0) + 30}], [0.5, 1], { extrapolateRight: 'clamp' });`;
    ...
    generateShapeElement(element) {
        const { id, props, timing } = element;
        const { shape = 'rectangle', width = 200, height = 200, x = '50%', y = '50%', backgroundColor = '#ff6b6b', borderColor = 'transparent', borderWidth = 0, borderRadius = 0, opacity = 1 } = props;
        const sequenceProps = timing
            ? `from={${timing.from}} durationInFrames={${timing.duration}}`
            : 'from={0} durationInFrames={90}';
        let shapeStyle = '';
    ...
    generateAnimationElement(element) {
        const { id, props, timing } = element;
        const { animationType = 'bounce', size = 50, color = '#4ecdc4', x = '50%', y = '50%' } = props;
        const sequenceProps = timing
            ? `from={${timing.from}} durationInFrames={${timing.duration}}`
            : 'from={0} durationInFrames={90}';
        let animationCode = '';
  ...
  const ${id}Rotation = safeInterpolate(frame, [${timing?.from || 0}, ${(timing?.from || 0) + 60}], [0, 360]);`;
    ...
    insertElementIntoRender(content, newElement, element) {
    ...
    replaceElementInRender(content, elementId, newElement) {
    ...
    async listElements(projectPath) {
        const compositionFile = path.join(projectPath, 'src', 'VideoComposition.tsx');
        if (!await fs_extra_1.default.pathExists(compositionFile)) {
            return 'No VideoComposition.tsx found in project';
        }
        const content = await fs_extra_1.default.readFile(compositionFile, 'utf8');
    ...
    async adjustTiming(projectPath, element) {
        const compositionFile = path.join(projectPath, 'src', 'VideoComposition.tsx');
        let content = await fs_extra_1.default.readFile(compositionFile, 'utf8');
        if (!element.timing) {
        ...
        const sequenceRegex = new RegExp(`(<Sequence[^>]*name="${element.id}"[^>]*)from={[0-9]+}([^>]*durationInFrames={[0-9]+}[^>]*>)`, 'g');
        content = content.replace(sequenceRegex, `$1from={${element.timing.from}}$2`);
        // Also update duration if provided
        const durationRegex = new RegExp(`(<Sequence[^>]*name="${element.id}"[^>]*from={[0-9]+}[^>]*)durationInFrames={[0-9]+}([^>]*>)`, 'g');
        content = content.replace(durationRegex, `$1durationInFrames={${element.timing.duration}}$2`);
        await fs_extra_1.default.writeFile(compositionFile, content);
...
exports.CompositionEditor = CompositionEditor;
/**
 * Register composition tools with the MCP server
 */
function registerCompositionTools(server) {
    const editor = new CompositionEditor(server);
    const logger = server.baseLogger.service('composition-tools');
    ...
    server.toolRegistry.registerTool({
        ...
        try {
            const result = await editor.executeAction(args);
            return {
                content: [{
                        type: 'text',
                        text: result
...
//# sourceMappingURL=composition-editor.js.map