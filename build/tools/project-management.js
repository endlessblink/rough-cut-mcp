...
export function createProjectManagementTools(config) {
...
export function createProjectManagementHandlers(config) {
    ...
    const handlers = {
        ...
        'analyze-video-structure': async (args) => {
        ...
        'launch-project-studio': async (args) => {
            ...
            try {
                ...
                const metadata = await projectMetadataManager.getOrCreateMetadata(projectPath, args.projectName, args.port || 7400 // Default port if not specified
                );
        ...
        'add-video-element': async (args) => {
            ...
            try {
                ...
                const newElement = createJSXElementFromTemplate(args.elementType, args.elementProps || {}, args.content ? [t.jsxText(args.content)] : undefined);
                // Add the element
                const addResult = addJSXElement(parseResult.ast, args.parentSelector || { elementName: 'AbsoluteFill' }, newElement, args.position || 'end');
        ...
        'remove-video-element': async (args) => {
        ...
        'delete-video-project': async (args) => {
        ...
        'replace-video-composition': async (args) => {
        ...
        'duplicate-video-project': async (args) => {
        ...
        'adjust-element-timing': async (args) => {
        ...
        'install-project-dependencies': async (args) => {
        ...
        'set-element-duration': async (args) => {
            ...
            try {
                if (args.elementId === 'composition') {
                    ...
                    const updatedContent = content.replace(/durationInFrames=\{(\d+)\}/g, `durationInFrames={${args.durationInFrames}}`);
        ...
        'set-element-position': async (args) => {
            ...
            try {
                ...
                if (args.x !== undefined && args.y !== undefined) {
                    ...
                    changes.style = {};
                    if (args.x !== undefined)
                        changes.style.left = `${args.x}px`;
                    if (args.y !== undefined)
                        changes.style.top = `${args.y}px`;
                }
                const editResult = editVideoElement(originalCode, {
                    elementId: args.elementId,
                    changes
        ...
        'set-element-size': async (args) => {
            ...
            try {
                ...
                if (args.width !== undefined)
                    changes.style.width = args.width;
                if (args.height !== undefined)
                    changes.style.height = args.height;
                const editResult = editVideoElement(originalCode, {
                    elementId: args.elementId,
                    changes
                });
        ...
        'rename-video-project': async (args) => {
        ...
        'generate-smart-project-name': async (args) => {
            ...
            try {
                ...
                if (/car|vehicle|drive|road/i.test(compositionCode))
                    keywords.push('car');
                if (/ball|bounce|physics/i.test(compositionCode))
                    keywords.push('ball');
                if (/text|title|subtitle|typography/i.test(compositionCode))
                    keywords.push('text');
                if (/particle|particles|stars?|dots?/i.test(compositionCode))
                    keywords.push('particles');
                if (/github|profile|avatar|user/i.test(compositionCode))
                    keywords.push('profile');
                if (/logo|brand/i.test(compositionCode))
                    keywords.push('logo');
                if (/3d|three|cube|sphere/i.test(compositionCode))
                    keywords.push('3d');
                if (/chart|graph|data|visualization/i.test(compositionCode))
                    keywords.push('data-viz');
                // Look for animation types
                if (/fade|opacity/i.test(compositionCode))
                    keywords.push('fade');
                if (/slide|translate/i.test(compositionCode))
                    keywords.push('slide');
                if (/scale|zoom/i.test(compositionCode))
                    keywords.push('zoom');
                if (/rotate|rotation|spin/i.test(compositionCode))
                    keywords.push('rotate');
                if (/bounce|spring|elastic/i.test(compositionCode))
                    keywords.push('bounce');
                // Look for style/theme
                if (/gradient|linear-gradient|radial-gradient/i.test(compositionCode))
                    keywords.push('gradient');
                if (/neon|glow/i.test(compositionCode))
                    keywords.push('neon');
                if (/dark|black|night/i.test(compositionCode))
                    keywords.push('dark');
                if (/minimal|simple|clean/i.test(compositionCode))
        ...
        'auto-rename-projects': async (args) => {
        ...
        'organize-projects-by-type': async (args) => {
            ...
            try {
                ...
                categories.forEach((cat) => organizationPlan[cat] = []);
                organizationPlan['uncategorized'] = [];
                for (const projectName of projects) {
                    // Skip if already in a category folder
                    if (categories.includes(projectName))
                        continue;
        ...
        'find-similar-projects': async (args) => {
                        ...
                        if (similarity >= threshold) {
                            ...
                            for (const group of similarGroups) {
                                if (group.projects.includes(sig1.name) || group.projects.includes(sig2.name)) {
                                    if (!group.projects.includes(sig1.name))
                                        group.projects.push(sig1.name);
                                    if (!group.projects.includes(sig2.name))
                                        group.projects.push(sig2.name);
        ...
        'add-project-metadata': async (args) => {
            ...
            try {
                ...
                if (args.autoGenerate) {
                    ...
                    const tags = [];
                    if (/animation/i.test(code))
                        tags.push('animation');
                    if (/3d|three/i.test(code))
                        tags.push('3d');
                    if (/particle/i.test(code))
                        tags.push('particles');
                    if (/text|typography/i.test(code))
                        tags.push('text');
                    if (/gradient/i.test(code))
                        tags.push('gradient');
                    if (/video/i.test(code))
                        tags.push('video');
                    if (/audio|sound/i.test(code))
                        tags.push('audio');
                    // Determine category
                    let category = 'general';
                    if (tags.includes('3d'))
                        category = '3d-animations';
                    else if (tags.includes('particles'))
                        category = 'effects';
...
async function getProjectDetails(projectPath, projectName) {
    const srcPath = path.join(projectPath, 'src');
    const videoCompositionPath = path.join(srcPath, 'VideoComposition.tsx');
    const compositionPath = path.join(srcPath, 'Composition.tsx');
    const packageJsonPath = path.join(projectPath, 'package.json');
