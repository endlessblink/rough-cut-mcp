...
export function createImageHandlers(config) {
    ...
    return {
        ...
        'generate-image-variations': async (args) => {
            if (!fluxService) {
            ...
            try {
                await fileManager.initializeDirectories();
                const results = await fluxService.generateVariations(args.prompt, args.count || 3, {
                    model: args.model,
                    width: args.width,
                    height: args.height,
        ...
        'generate-styled-image': async (args) => {
            if (!fluxService) {
            ...
            try {
                await fileManager.initializeDirectories();
                const result = await fluxService.generateWithStyle(args.prompt, args.style, {
                    model: args.model,
                    width: args.width,
                    height: args.height,
        ...
        'batch-generate-images': async (args) => {
            if (!fluxService) {
        ...
        'optimize-image-prompt': async (args) => {
            if (!fluxService) {
            ...
            try {
                ...
                function getPromptImprovements(original, optimized) {
                    const improvements = [];
