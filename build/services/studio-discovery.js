...
exports.StudioDiscoveryService = void 0;
const child_process_1 = require("child_process");
const util_1 = require("util");
const logger_js_1 = require("../utils/logger.js");
const execAsync = (0, util_1.promisify)(child_process_1.exec);
class StudioDiscoveryService {
    logger = (0, logger_js_1.getLogger)().service('StudioDiscovery');
    defaultOptions = {
     ...
     */
    async discoverRunningStudios(options) {
     ...
     */
    async identifyStudio(port, options) {
        const opts = { ...this.defaultOptions, ...options };
        try {
     ...
     */
    async findBestStudio(preferredProject) {
        ...
        if (preferredProject) {
            const matching = studios.find(s => s.isHealthy &&
     ...
     */
    async getListeningPorts(portRange) {
     ...
     */
    async getProcessInfoFromPort(port) {
        const platform = process.platform;
        try {
            if (platform === 'win32') {
     ...
     */
    async performHealthCheck(port, timeout = 3000) {
        try {
     ...
     */
    async pingStudio(port, timeout = 2000) {
        try {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), timeout);
            const response = await fetch(`http://localhost:${port}`, {
     ...
     */
    async isStudioRunningProject(port, projectName) {
        try {
...
exports.StudioDiscoveryService = StudioDiscoveryService;
//# sourceMappingURL=studio-discovery.js.map