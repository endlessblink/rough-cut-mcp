...
class StudioLifecycle {
    ...
    static async validateProjectPath(projectPath) {
        try {
            ...
            const remotionConfigPath = (0, path_1.join)(windowsPath, 'remotion.config.ts');
            try {
                const configContent = await fs_1.promises.readFile(remotionConfigPath, 'utf-8');
            ...
            const entryPointPath = (0, path_1.join)(windowsPath, 'src', 'index.ts');
            try {
                const entryContent = await fs_1.promises.readFile(entryPointPath, 'utf-8');
            ...
            const rootComponentPath = (0, path_1.join)(windowsPath, 'src', 'Root.tsx');
            try {
                await fs_1.promises.access(rootComponentPath);
            }
            catch {
