...
exports.processImportsAndSyntax = processImportsAndSyntax;
exports.validateCompositionUsage = validateCompositionUsage;
/**
 ...
 */
function addMissingImports(code, missingImports, existingImports) {
    ...
    for (const [packageName, functions] of importsByPackage) {
        ...
        if (existingPackageImports) {
            ...
            const existingImportRegex = new RegExp(`import\\s*\\{\\s*([^}]+)\\s*\\}\\s*from\\s*['"]${packageName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"];?`, 'g');
 ...
 */
function validateCompositionUsage(code) {
    const errors = [];
    const suggestions = [];
