...
exports.SAFE_CSS_TEMPLATE = void 0;
exports.validateJSXSyntax = validateJSXSyntax;
exports.processJSXSyntax = processJSXSyntax;
exports.validateCSSInJS = validateCSSInJS;
/**
...
function validateJSXSyntax(code) {
    ...
    while ((braceMatch = styleBraceRegex.exec(code)) !== null) {
        ...
        if (styleContent.includes(':') && !styleContent.startsWith('{')) {
            ...
            correctedCode = correctedCode.replace(`style=${styleContent}>`, `style={{${styleContent}}}>`);
        }
    }
    ...
    while ((interpolationMatch = interpolationRegex.exec(code)) !== null) {
        const context = code.substring(Math.max(0, interpolationMatch.index - 20), Math.min(code.length, interpolationMatch.index + interpolationMatch[0].length + 20));
 ...
 */
function processJSXSyntax(code) {
 ...
 */
function validateCSSInJS(cssText) {
    const errors = [];
...
`;
//# sourceMappingURL=jsx-syntax-validator.js.map