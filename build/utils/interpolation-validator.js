 ...
 */
function isMonotonicallyIncreasing(arr) {
 ...
 */
function validateInterpolationRangeLegacy(range) {
...
function processVideoCode(code) {
    ...
    processedCode = processedCode.replace(interpolateRegex, (match, frame, inputStr, outputStr, options) => {
        try {
            ...
            const outputRange = outputStr.split(',').map((s) => parseFloat(s.trim()));
 ...
 */
exports.SAFE_INTERPOLATE_HELPER = `
// Helper to ensure interpolation ranges are valid (prevents Remotion errors)
function validateRange(range) {
  if (range.length <= 1) return range;
  const valid = [...range];
...
function runValidationTests() {
    ...
    let passed = 0;
    let failed = 0;
    exports.TEST_CASES.forEach((testCase, index) => {
