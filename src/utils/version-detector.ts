 ...
 */
export async function getParentRemotionVersions(): Promise<RemotionVersions> {
  try {
 ...
 */
export async function generateSafeDependencies(): Promise<any> {
 ...
 */
export async function checkProjectVersionConflicts(projectPath: string): Promise<{
  ...
  try {
    ...
    for (const pkg of packagesToCheck) {
      ...
      const expectedVersion = pkg === 'remotion' 
        ...
        : parentVersions[pkg.split('/')[1] as keyof RemotionVersions];
 ...
 */
export async function repairProjectVersions(projectPath: string): Promise<void> {
