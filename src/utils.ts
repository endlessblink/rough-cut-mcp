...
export async function getMCPStatusInfo(): Promise<any> {
  try {
    const currentVersion = '9.5.0'; // Intelligent Artifact Classification - Prevents Content Destruction
    const serverDir = __dirname.endsWith('build') ? path.dirname(__dirname) : __dirname;
    ...
    let npmRegistryVersion = '9.5.0'; // Known published version
    ...
    console.error('[MCP-STATUS] Using cached registry version: 9.5.0');
