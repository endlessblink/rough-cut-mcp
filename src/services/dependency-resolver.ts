...
export class DependencyResolver {
    ...
    try {
      ...
      if (availableNodes) {
        for (const [nodeId, deps] of result.allDependencies) {
          const missing = Array.from(deps).filter(
            dep => !availableNodes.has(dep) && !nodeIds.includes(dep)
          );
          if (missing.length > 0) {
      ...
      if (circulars.length > 0) {
        ...
        result.errors.push(
          `Circular dependencies detected: ${circulars
            .map(([a, b]) => `${a} <-> ${b}`)
            .join(', ')}`
        );
        return result;
   ...
   */
  private topologicalSort(nodeIds: string[]): string[] | null {
    // Build subgraph with all required nodes
    const requiredNodes = new Set<string>(nodeIds);
    for (const nodeId of nodeIds) {
      const deps = this.getTransitiveDependencies(nodeId);
  ...
  getStatistics(): {
    ...
    maxDependencies: { id: string; count: number };
    maxDependents: { id: string; count: number };
