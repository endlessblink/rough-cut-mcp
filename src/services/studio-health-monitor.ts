...
const logger = getLogger().service('StudioHealthMonitor');
...
export class StudioHealthMonitor extends EventEmitter {
  private config: HealthMonitorConfig;
  private isRunning: boolean = false;
  private healthCheckInterval?: NodeJS.Timeout;
  private studioHealth = new Map<number, StudioHealthStatus>();
  private recoveryQueue = new Map<number, { attempts: number; nextAttempt: Date }>();
   ...
   */
  private async checkStudioHealth(studio: StudioProcess): Promise<void> {
    ...
    try {
      ...
      const healthStatus: StudioHealthStatus = {
        port,
        pid: studio.pid,
   ...
   */
  private queueForRecovery(port: number): void {
    const existingHealth = this.studioHealth.get(port);
   ...
   */
  private async attemptStudioRecovery(port: number): Promise<void> {
    ...
    try {
      ...
      if (!shutdownResult.success) {
        ...
        await StudioLifecycle.shutdownStudios({
          port,
          force: true
        });
      }
   ...
   */
  async triggerRecovery(port: number): Promise<void> {
    const healthStatus = this.studioHealth.get(port);
    if (!healthStatus) {
