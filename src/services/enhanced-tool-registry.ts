...
export class EnhancedToolRegistry extends ToolRegistry {
  ...
  private enhancedConfig: EnhancedRegistryConfig;
  // Remove duplicate logger - use inherited one
  private toolToLayer: Map<string, string>;
  ...
  private setupDefaultLayers(): void {
    ...
    for (const [categoryId, categoryInfo] of Object.entries(TOOL_CATEGORIES)) {
      const layer: ToolLayer = {
        ...
        categories: [categoryId as ToolCategory],
  ...
  registerTool(tool: Tool, handler: Function, metadata: ToolMetadata): void {
    ...
    if (this.dependencyResolver && metadata.dependencies) {
      this.dependencyResolver.registerNode(
        tool.name,
    ...
    if (this.contextManager) {
      this.contextManager.addItem(
        tool.name,
        'tool',
        metadata.estimatedTokens || 100,
        metadata.priority || 5,
        metadata.loadByDefault
      );
    }
   ...
   */
  async activateToolsEnhanced(request: ToolActivationRequest): Promise<{
    success: boolean;
    activated: string[];
    deactivated: string[];
    dependencies: string[];
    ...
    try {
      ...
      if (this.auditLogger) {
        for (const toolName of result.activated) {
          this.auditLogger.logToolActivation(
            toolName,
            'user',
            { 
   ...
   */
  private handleLayerChange(event: any): void {
   ...
   */
  private handleContextPressure(event: any): void {
   ...
   */
  private handleContextOptimization(event: any): void {
    ...
    for (const toolName of event.removed) {
      ...
      if (this.auditLogger) {
        this.auditLogger.logToolDeactivation(
          toolName,
          'system',
