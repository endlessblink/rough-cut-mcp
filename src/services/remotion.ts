...
export class RemotionService {
  ...
  private logger = getLogger().service('Remotion');
  ...
  async createVideo(
    ...
    try {
      ...
      const indexCode = generateIndexFile(
        request.duration || 30,
        request.fps || 30,
        request.dimensions || { width: 1920, height: 1080 }
      );
...
export const TextComposition: React.FC = () => {
   ...
   */
  async testRemotionSetup(): Promise<boolean> {
    try {
...
registerRoot(() => (
  ...
  async createStudioProject(
    ...
    const videoCode = generateIndexFile(
      request.duration || 30,
      request.fps || 30,
      request.dimensions || { width: 1920, height: 1080 }
    );
...
registerRoot(RemotionVideo);`;
   ...
   */
  async cleanupOldRenders(maxAgeHours: number = 24): Promise<void> {
    const videoDir = getAssetPath(this.config, 'videos');
    const tempDir = getAssetPath(this.config, 'temp');
