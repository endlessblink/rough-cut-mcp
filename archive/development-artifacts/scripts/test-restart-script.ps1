...
function Test-ClaudeCache {
    Write-Host "🗑️  Checking Claude Desktop cache locations..." -ForegroundColor Yellow
...
try {
    ...
    if ($processes.Count -gt 0) {
        Write-Host "✅ Would stop $($processes.Count) Claude processes" -ForegroundColor Green
    } else {
        Write-Host "ℹ️  No Claude processes currently running" -ForegroundColor Blue
    }
    
    if ($caches.Count -gt 0) {
        Write-Host "✅ Would clear $($caches.Count) cache locations" -ForegroundColor Green  
    } else {
        Write-Host "ℹ️  No cache directories found" -ForegroundColor Blue
    }
    
    if ($executable) {
        Write-Host "✅ Would restart Claude from: $executable" -ForegroundColor Green
    } else {
        Write-Host "⚠️  Could not find Claude executable - manual start needed" -ForegroundColor Yellow
    }
    
    Write-Host ""
    Write-Host "🚀 READY TO RUN SAFELY!" -ForegroundColor Green
    Write-Host "To actually execute: .\restart-claude-clean.ps1 -Force" -ForegroundColor White
