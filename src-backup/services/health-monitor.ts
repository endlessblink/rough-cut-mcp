...
export class HealthMonitor {
  ...
  private logger = getLogger().service('HealthMonitor');
   ...
   * Check filesystem health
   */
  private async checkFilesystem(issues: HealthIssue[]): Promise<void> {
    try {
   ...
   * Check system resources
   */
  private async checkSystemResources(issues: HealthIssue[]): Promise<void> {
    try {
      // Check memory usage
   ...
   */
  private async checkDependencies(issues: HealthIssue[]): Promise<void> {
    try {
   ...
   * Check external services
   */
  private async checkExternalServices(services: ServiceHealth[]): Promise<void> {
    // ElevenLabs API
    if (this.config.apiKeys.elevenlabs) {
      const elevenLabsHealth = await this.checkService(
        'ElevenLabs',
    ...
    if (this.config.apiKeys.freesound) {
      const freesoundHealth = await this.checkService(
        'Freesound',
        'https://freesound.org/apiv2/search/text/?query=test&token=' + this.config.apiKeys.freesound
      );
    ...
    if (this.config.apiKeys.flux) {
      const fluxHealth = await this.checkService(
        'Flux AI',
        'https://api.flux.ai/v1/models', // This might not be the real endpoint
   ...
   */
  private async checkRemotionHealth(issues: HealthIssue[]): Promise<void> {
    try {
   ...
   */
  private calculateHealthScore(issues: HealthIssue[], services: ServiceHealth[]): number {
    let score = 100;
   ...
   */
  private determineOverallStatus(score: number, issues: HealthIssue[]): 'healthy' | 'degraded' | 'unhealthy' {
  ...
  getHealthTrends(): {
    ...
    commonIssues: { component: string; count: number }[];
  } {
