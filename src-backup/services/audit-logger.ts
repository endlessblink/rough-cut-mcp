...
export interface AuditStatistics {
  ...
  peakPeriods: Array<{ hour: number; count: number }>;
}
...
export class AuditLogger extends EventEmitter {
  ...
  private persistInterval: NodeJS.Timeout | null;
  private entryIndex: Map<string, Set<string>>; // Index by entity
   ...
   */
  private detectActivationCycles(entries: AuditEntry[]): AuditPattern | null {
    const entityCycles = new Map<string, number>();
   ...
   */
  private detectErrorClusters(entries: AuditEntry[]): AuditPattern | null {
    const errorGroups: AuditEntry[][] = [];
   ...
   */
  private detectPerformanceDegradation(entries: AuditEntry[]): AuditPattern | null {
   ...
   */
  private detectContextThrashing(entries: AuditEntry[]): AuditPattern | null {
