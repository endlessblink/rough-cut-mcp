...
import { FluxImageResponse } from '../types/api-responses.js';
import { getAssetPath } from '../utils/config.js';
import { getLogger } from '../utils/logger.js';
import { validateTextContent, validateFileExtension, ImageExtensions } from '../utils/validation.js';

export class FluxService {
  private client: AxiosInstance;
  private config: MCPConfig;
  private logger = getLogger().service('Flux');
  ...
  async generateImage(request: ImageGenerationRequest): Promise<ImageGenerationResult> {
    ...
    try {
      ...
      // Poll for completion
      let imageData: FluxImageResponse;
      let attempts = 0;
  ...
  async generateVariations(
    ...
    const results = await Promise.allSettled(
      variations.map(variation => this.generateImage(variation))
    );

    const successful: ImageGenerationResult[] = [];
    const failures: any[] = [];
