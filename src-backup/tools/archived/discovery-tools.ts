 ...
 */
export function createDiscoveryTools(registry: ToolRegistry): Tool[] {
 ...
 */
export function createDiscoveryHandlers(registry: ToolRegistry): Record<string, Function> {
  const logger = getLogger().service('DiscoveryHandlers');

  return {
    'discover-capabilities': async (args: any) => {
      try {
    ...
    'activate-toolset': async (args: any) => {
      try {
    ...
    'get-active-tools': async (args: any) => {
      try {
        const activeTools = registry.getActiveTools();
        
        const tools = activeTools.map(tool => {
          const extendedTool = tool as any;
          const base = {
            name: tool.name,
            description: tool.description,
          };
    ...
    'get-tool-usage-stats': async (args: any) => {
      try {
        const stats = registry.getUsageStatistics();
 ...
 */
export function getDiscoveryToolsMetadata(): ToolMetadata[] {
  return [
    {
      name: 'discover-capabilities',
