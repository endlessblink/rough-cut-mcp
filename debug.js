  ...
  try {
    ...
    if (fs.existsSync(configFile)) {
      ...
      try {
        ...
        if (config.mcpServers && config.mcpServers.remotion) {
          log('green', '✅ Remotion MCP server found in config');
          
          const remotionConfig = config.mcpServers.remotion;
          log('blue', `Command: ${remotionConfig.command}`);
          log('blue', `Args: ${JSON.stringify(remotionConfig.args)}`);
          
          // Verify the configured paths exist
          if (fs.existsSync(remotionConfig.command)) {
            log('green', '✅ Node.js command path exists');
          } else {
            log('red', `❌ Node.js command path not found: ${remotionConfig.command}`);
          }
          
          if (remotionConfig.args && remotionConfig.args[0] && fs.existsSync(remotionConfig.args[0])) {
            log('green', '✅ MCP server path exists');
          } else {
            log('red', `❌ MCP server path not found: ${remotionConfig.args?.[0]}`);
          }
          
        } else {
          log('red', '❌ Remotion MCP server not found in config');
